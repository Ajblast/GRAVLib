...
GRAVLib::Concurrency::Jobs::counter::~counter()
{
...
GRAVLib::Concurrency::Jobs::counterTarget GRAVLib::Concurrency::Jobs::counter::increment()
{
...
GRAVLib::Concurrency::Jobs::counterTarget GRAVLib::Concurrency::Jobs::counter::decrement()
{
	m_DestructionLock.fetch_add(1, std::memory_order_seq_cst);
...
GRAVLib::Concurrency::Jobs::counterTarget GRAVLib::Concurrency::Jobs::counter::getValue()
{
	m_DestructionLock.fetch_add(1, std::memory_order_seq_cst);
...
bool GRAVLib::Concurrency::Jobs::counter::addWaitingFiber(Fibers::fiberID id, counterTarget target)
{
	...
	throw GRAVLib::Concurrency::Jobs::Exceptions::counterWaitListFullException("Job Counter waiting slots full");
}

void GRAVLib::Concurrency::Jobs::counter::checkWaitingFibers(counterTarget value)
{
	GRAVLib_PROFILE_FUNCTION();
	...
	for (size_t i = 0; i < m_FiberCount; i++)
	{
		...
		if (m_WaitingFibers[i].m_TargetValue == value)
		{
			...
			jobManager::getInstance().addReadyFiber(m_WaitingFibers[i].m_ID);
