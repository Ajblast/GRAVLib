...
namespace GRAVLib::Utils
{
	...
	template<typename char_t>
	GRAVLibAPI inline char_t toLower(char_t in)
	{
		return isUpper(in) ? (char_t)(in + 0x20) : in;
	}
	// Get the uppercase of an ASCII character
	template<typename char_t>
	GRAVLibAPI inline char_t toUpper(char_t in)
	{
	...
	GRAVLibAPI inline void trimLeft(std::string& str, const std::string& delimiters = " \f\n\r\t\v")
	{
		str.erase(0, str.find_first_not_of(delimiters));
	}
	// Trim the right of a string
	GRAVLibAPI inline void trimRight(std::string& str, const std::string& delimiters = " \f\n\r\t\v")
	{
		str.erase(str.find_last_not_of(delimiters) + 1);
	}
	// Trim the left and right of a string
	GRAVLibAPI inline void trim(std::string& str, const std::string& delimiters = " \f\n\r\t\v")
	{
	...
	GRAVLibAPI inline void split(const std::string& str, const std::string& delimiter, std::vector<std::string>& tokens)
	{
		size_t start = 0;
		size_t end;
	...
	GRAVLibAPI inline void trimSplit(const std::string& str, const std::string& delimiter, std::vector<std::string>& tokens, const std::string& trimDelimiter = " \f\n\r\t\v")
	{
		// Split the string
