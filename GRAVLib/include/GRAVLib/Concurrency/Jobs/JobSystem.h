...
#define GRAVLib_KICK_JOB(declaration, counter)			GRAVLib::Concurrency::Jobs::jobManager::getInstance().kickJob(declaration, counter)
#define GRAVLib_KICK_JOBS(declarations, size, counter)	GRAVLib::Concurrency::Jobs::jobManager::getInstance().kickJobs(declarations, size, counter)
#define GRAVLib_WAIT_COUNTER(counter, target)			GRAVLib::Concurrency::Jobs::jobManager::getInstance().waitForCounter(counter, target)
